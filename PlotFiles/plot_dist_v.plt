...
set ylabel "$d_v/\\|\\bm{v}_{\\mathcal{H}}\\|,\\, \\%$"
set xrange [-180:180]
set xtics 45
set yrange [0:4]
# set ytics 0, 1, 6
...
plot filename5 using 1:($5*100) w linespoints ls 5 title "MLP, 10"
replot filename6 using 1:($5*100) w linespoints ls 6 title "RNN, 10"
replot filename3 using 1:($5*100) w linespoints ls 3 title "MLP, 5"
replot filename4 using 1:($5*100) w linespoints ls 4 title "RNN, 5"
replot filename1 using 1:($5*100) w linespoints ls 1 title "MLP, 1"
replot filename2 using 1:($5*100) w linespoints ls 2 title "RNN, 1"



set terminal epslatex standalone color colortext 12 lw 2 header \
"\\usepackage{amsmath}\n\\usepackage{bm}\n\\usepackage{siunitx}"
set output 'dist_v.tex'
replot