...
def CR3BP_equations_controlled_ivp(t, state, data):  #with control (with ivp solver)
    '''
    input: -t (time)
           -state
           -F (thrust)
...
def Jacobi_constant(x, y, z, vx, vy, vz):
    ...
    C=2*(((1-mu)/r13)+mu/r23+0.5*(x**2+y**2+z**2))-(vx**2+vy**2+vz**2)  #Jacobi constant
...
def min_dist_v(t, y, data):
    ...
    #sc absolute and relative velocities (wrt Halo)
    v_sc = data[3:6]  #absolute sc velocity
